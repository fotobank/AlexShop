...
class OrderAdmin extends Simpla
{
    public function fetch()
    {
            ...
            if ($order->id){
                ...
                foreach ($purchases as $purchase){
                    ...
                    if (!empty($purchase->id))
                        if (!empty($variant))
                            $this->orders->update_purchase($purchase->id, ['variant_id' => $purchase->variant_id, 'variant_name' => $variant->name, 'sku' => $variant->sku, 'price' => $purchase->price, 'amount' => $purchase->amount]);
                        else
                            $this->orders->update_purchase($purchase->id, ['price' => $purchase->price, 'amount' => $purchase->amount]);
                    elseif (!$purchase->id = $this->orders->add_purchase([
                        'order_id' => $order->id,
                ...
                foreach ($this->orders->get_purchases(['order_id' => $order->id]) as $p)
                ...
                if ($new_status == 0){
                    if (!$this->orders->open((int)($order->id)))
                        ...
                        $this->orders->update_order($order->id, ['status' => 0]);
                } elseif ($new_status == 1) {
                    if (!$this->orders->close((int)($order->id)))
                        $this->design->assign('message_error', 'error_closing');
                    else
                        $this->orders->update_order($order->id, ['status' => 1]);
                } elseif ($new_status == 2) {
                    if (!$this->orders->close((int)($order->id)))
                        $this->design->assign('message_error', 'error_closing');
                    else
                        $this->orders->update_order($order->id, ['status' => 2]);
                } elseif ($new_status == 3) {
                    if (!$this->orders->open((int)($order->id)))
                        $this->design->assign('message_error', 'error_open');
                    else
        ...
        $subtotal = 0;
        $purchases_count = 0;
        if ($order && $purchases = $this->orders->get_purchases(['order_id' => $order->id])){
            // Покупки
            $products_ids = [];
            $variants_ids = [];
            foreach ($purchases as $purchase){
            ...
            $variants = [];
            foreach ($this->variants->get_variants(['product_id' => $products_ids]) as $v)
                $variants[$v->id] = $v;
