...
if ($c = $simpla->request->get('code')){
    ...
    if ($new_password = $simpla->request->post('new_password')){
        ...
        if (!is_writable($passwd_file)){
            ...
            if (!$simpla->managers->update_manager($new_login, ['password' => $new_password]))
                $simpla->managers->add_manager(['login' => $new_login, 'password' => $new_password]);
    ...
    if (isset($_POST['email'])){
        if ($_POST['email'] === $admin_email){
            $code = $simpla->config->token(random_int(1, mt_getrandmax()) . random_int(1, mt_getrandmax()) . random_int(1, mt_getrandmax()));
